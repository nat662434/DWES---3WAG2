...
<table border=1>
    ...
    <?php
    foreach ($pertsonak as $pertsona => $data) {
        if (substr($pertsona, 0, 1) == "M") { ?>
            <tr>
                <td><?php echo $pertsona ?></td>
                <td><?php echo $data['abizena'] ?></td>
                <td><?php echo $data['adina']?></td>
                <td><?php echo $data['soldata'] ?></td>
            </tr>
            <?php 
            $balioak = '"'.$pertsona.'", "'.$data['abizena'].'", "'.$data['adina'].'", "'.$data['soldata'].'"';
            // echo $balioak."<br>";
            $sql = "INSERT INTO a1 (izena, abizenak, adina, soldata) VALUES ($balioak);";
            // echo $sql."<br>";
            $db->insert($sql);
        }
    }
