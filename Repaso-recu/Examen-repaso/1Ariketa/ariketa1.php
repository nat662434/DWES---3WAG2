...
<table border=1>
    ...
    <?php foreach ($pertsonak as $pertsona => $item) {
        if (substr($pertsona, 0, 1) == 'M'){?>
            <tr>
                <td><?php echo $pertsona ?></td>
                <td><?php echo $item["abizena"] ?></td>
                <td><?php echo $item["adina"] ?></td>
                <td><?php echo $item["soldata"] ?></td>
                <?php $sql = "INSERT INTO pertsonak (izena, abizenak, adina, soldata) VALUES ('$pertsona', '{$item['abizena']}', '{$item['adina']}', '{$item['soldata']}')";
                $db->insert($sql);?>
            </tr>
        <?php } 
    }?>
